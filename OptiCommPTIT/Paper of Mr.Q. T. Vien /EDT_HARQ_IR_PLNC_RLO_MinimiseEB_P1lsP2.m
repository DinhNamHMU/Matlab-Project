...
for p1_idx=1:length(P1)
    for l=1:Loop
        ...
        % energy per bit
        EB1 = (P1(p1_idx).*max(KMACa,K11a)+P2(p1_idx).*max(KMACa,K22a)+PR(p1_idx).*K2a)/(2*R);
        ...
        % effective delay
        D1 = (K1a(alpha1min_idx,d1R1min_idx)+K2a(alpha1min_idx,d1R1min_idx))./(2*R);
        ...
        % energy per bit
        EB2 = (P1(p1_idx).*max(KMACb,K11b)+P2(p1_idx).*max(KMACb,K22b)+PR(p1_idx).*K2b)/(2*R);
        ...
        % effective delay
        D2 = (K1b(alpha2min_idx,d1R2min_idx)+K2b(alpha2min_idx,d1R2min_idx))./(2*R);
