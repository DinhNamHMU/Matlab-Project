...
for p1_idx=1:length(P1)
    for l=1:Loop
        ...
        % effective delay
        D = (max(max(KANC1,K11),max(KANC2,K22))+max(KANC1,KANC2))./(2*R);
        
        % find relay location to minimize D        
        [Dmintemp,d0Rmin_idx] = min(D);
        Dmin(p1_idx) = Dmin(p1_idx) + Dmintemp;
        d0Rmin = d0R(d0Rmin_idx);
        d1Rmin(p1_idx) = d1Rmin(p1_idx) + sqrt(d0Rmin^2+(d2^2)/4);
        alphamin(p1_idx) = alphamin(p1_idx) + atan((d2/2)./d0Rmin);        
        % energy per bit
        EB(p1_idx) = EB(p1_idx) + (P1(p1_idx).*max(KANC1(d0Rmin_idx),K11(d0Rmin_idx))+P2(p1_idx).*max(KANC2(d0Rmin_idx),K22(d0Rmin_idx))+(PR(p1_idx)/2).*KANC1(d0Rmin_idx)+(PR(p1_idx)/2).*KANC2(d0Rmin_idx))./(2*R);
