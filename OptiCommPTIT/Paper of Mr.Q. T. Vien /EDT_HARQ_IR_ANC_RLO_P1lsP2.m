...
% relay location
% alpha = angle (D1,S1,R)
alpha11 = 0; % limit 1 for scenario a
alpha12 = atan((d2/2)/d1); % limit 2 for scenario a
alpha1 = [alpha11+(alpha12-alpha11)/10:(alpha12-alpha11)/10:alpha12-(alpha12-alpha11)/10]';

alpha21 = atan((d2/2)/d1); % limit 1 for scenario b
alpha22 = pi/2; % limit 2 for scenario b
alpha2 = [alpha21+(alpha22-alpha21)/10:(alpha22-alpha21)/10:alpha22-(alpha22-alpha21)/10]';

% d1R = distance (S1->R)
d1R11 = zeros(length(alpha1),1);; % limit 1 for scenario a
d1R12 = d1./cos(alpha1); % limit 2 for scenario a

d1R21 = zeros(length(alpha2),1);; % limit 1 for scenario b
d1R22 = (d2/2)./sin(alpha2); % limit 1 for scenario b
...
for p1_idx=1:length(P1)
    for l=1:Loop
        ...
        % effective delay
        D1 = (max(max(KANC1a,K11a),max(KANC2a,K22a))+max(KANC1a,KANC2a))./(2*R);
        ...
        % energy per bit
        EB1 = (P1(p1_idx).*max(KANC1a(alpha1min_idx,d1R1min_idx),K11a(alpha1min_idx,d1R1min_idx))+P2(p1_idx).*max(KANC2a(alpha1min_idx,d1R1min_idx),K22a(alpha1min_idx,d1R1min_idx))+(PR(p1_idx)/2).*KANC1a(alpha1min_idx,d1R1min_idx)+(PR(p1_idx)/2).*KANC2a(alpha1min_idx,d1R1min_idx))./(2*R);        
        ...
        % effective delay
        D2 = (max(max(KANC1b,K11b),max(KANC2b,K22b))+max(KANC1b,KANC2b))./(2*R);
        ...
        % energy per bit
        EB2 = (P1(p1_idx).*max(KANC1b(alpha2min_idx,d1R2min_idx),K11b(alpha2min_idx,d1R2min_idx))+P2(p1_idx).*max(KANC2b(alpha2min_idx,d1R2min_idx),K22b(alpha2min_idx,d1R2min_idx))+(PR(p1_idx)/2).*KANC1b(alpha2min_idx,d1R2min_idx)+(PR(p1_idx)/2).*KANC2b(alpha2min_idx,d1R2min_idx))./(2*R);        
