function [keyLossRate]=calculateKeyLossRate(M,B,QBER,P_sift) 
    %Simulation Parameters 
    global Rb;                             %Bit rate (bps)
    global lamda_wavelength;               %Wavelength (m)
    global v_wind;                         %Wind speed (m/s)
    global lamda;                          %Flow throughput(bit strings per second)
    global H_P;                            %Amospheric altitude (m)
    global l_k;                            %Length of bit string(bits)
                                
    t_o=sqrt(lamda_wavelength*H_P)/v_wind; %Turbulence coherent time(second)
