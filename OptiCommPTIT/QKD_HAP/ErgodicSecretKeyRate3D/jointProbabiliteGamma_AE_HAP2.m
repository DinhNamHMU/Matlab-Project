function jointProbabiliteGamma=jointProbabiliteGamma_AE_HAP2(bit_Alice,bit_Eve,X_r_Array)
    ...
    %Eve (HAP of Vehicle)
    G_RX_E_2_dB=121;           %RX Telescope Gain (Scenario 1)
    G_RX_E_2=10.^(G_RX_E_2_dB./10);
    ...
    h_p_G_E=A_0_G.*exp(-2.*(X_r_Array.^2)./Omega_zeq_G_2);
    ...
    %Peak received power at Eve
    P_r_E=1./FSL.*P.*G_TX_S.*h_p_P.*G_RX_P.*G_A.*G_TX_P.*h_p_G_E.*G_RX_E_2.*hl;
    ...
    for i=1:length(t)
        a(i)=teta(i)./temp1;   
        for l=1:length(t)
            i0=-1./(4.*special(i)).*P_r_E.*ModDepth.*t(l);
            i1=-i0;

            sigmaN_i_l=sqrt(2.*q.*R.*M.*FA.*(1./(4.*special(i)).*P_r_E.*R.*M.*ModDepth.*t(l)+1./special(i).*P_b_P.*G_RX_P.*...
                       G_A.*G_TX_P.*h_p_G_E.*G_RX_E_2.*hl.*t(l)+1./special(i).*P_a.*G_TX_P.*h_p_G_E.*G_RX_E_2.*hl.*t(l)+...
                       P_b_G.*G_RX_E_2).*deltaf+4.*kB.*T.*Fn.*deltaf./RL);
