...
Wo=0.1;%Wo=0.1;% spot radius!!!; waist=0.0329, to ensure that the spot radius is larger than the waist spot radius
divergence_angle= 30.*10.^(-6);%linspace(5,80,21).*10.^(-6);%

%Fo = z./(1-((z.*divergence_angle./Wo).^2-(2.*z./(k.*Wo.^2)).^2).^(0.5));
Waist=2.*wavelength./(pi.*divergence_angle);%ÊøÑü°ë¾¶ 
...
delta11=(G.*e).^2.*F.*(Kb+Ks)+deltaTT;
 ...
 delta1=sqrt(delta11);
 %delta0=sqrt(delta00);   % formula modification will make the Gaussian noise variance in the integration
