...
h0=100;
z = (H-h0).*sec(zenith);                     % distance of transmition
k = 2.*pi./wavelength;                       % number of wave
Wo=0.1;                                      % spot radius!!! ; waist=0.0329, Ensure that the spot radius is greater than the waist radius
...
dpsk=0;ttt=1;
for ttt=1:1:2
deltaf=linspace(d1(ttt),d1(ttt)+pi,100);% to be determined
fgvar=2*pi.*fIF.*Ts;
%fai=0;
...
for tt=1:100
    ...
    delta1=sqrt(delta11);
    % formula modification will make the Gaussian noise variance in the integration
