function Result=jointProbabiliteLogEve3D(bit_Alice,bit_Eve,X_ModDepthArray,Y_gArray) 
   %Log-normal channels
    ...
    %Calculate QBER
    for i=1:length(x)
        FA=kA.*Y_gArray+(2-1./Y_gArray).*(1-kA);             %Excess noise factor
        ...
        i0=-1./4.*R.*Y_gArray.*P.*X_ModDepthArray.*hl.*exp(2.*sqrt(2).*sqrt(sigmaX2).*x(i)+2.*uX);
        i1=-i0;
        
        sigmaN_i=sqrt(2.*q.*FA.*(Y_gArray.^2).*R.*(1./4.*P.*X_ModDepthArray.*hl.*exp(2.*sqrt(2).*sqrt(sigmaX2).*x(i)+2.*uX)+Pb).*deltaf+(4.*kB.*T.*Fn.*deltaf)./RL);
