function [Result]=jointProbabiliteGammaEve3D(bit_Alice,bit_Eve,X_LArray,Y_ModDepthArray) 
    ...
    X_LArray=X_LArray.*10^3;
    hl=A./(pi.*(tetaAngle./2.*X_LArray).^2).*exp(-betal.*X_LArray./1000);
    
    sigmaR2=1.23.*(2.*pi./lamda).^(7/6).*X_LArray.^(11/6).*C2n;
    ...
    for i=1:length(t)
        a(:,:,i)=teta(:,:,i)./temp1;     
        for l=1:length(t)
            i0=-1./(4.*special(:,:,i)).*R.*g.*P.*Y_ModDepthArray.*hl.*t(l);
            i1=-i0;
            
            sigmaN_i_l=sqrt(2.*q.*FA.*g.^2.*R.*(1./(4.*special(:,:,i)).*P.*Y_ModDepthArray.*hl.*t(l)+Pb).*deltaf+4.*kB.*T.*Fn.*deltaf./RL);
            
            dE=0;
            
            if bit_Alice==1 && bit_Eve==0
