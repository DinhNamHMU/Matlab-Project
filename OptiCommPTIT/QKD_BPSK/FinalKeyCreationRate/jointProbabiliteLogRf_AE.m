function [Result]=jointProbabiliteLogRf_AE(bit_Alice,bit_Eve,P_dBm) 
    %Log-normal channels
    ...
    L=1500;                   %Transmission distance(m)
    ...
    for i=1:length(x)  
        ...
        if bit_Alice==0 && bit_Eve==0
            Sum(i)=1/(2*sqrt(pi))*w(i)*qfunc((i0-dE)./sigmaN_i);
        elseif bit_Alice==1 && bit_Eve==0
            Sum(i)=1/(2*sqrt(pi))*w(i)*qfunc((i1-dE)./sigmaN_i);
        elseif bit_Alice==0 && bit_Eve==1
            Sum(i)=1/(2*sqrt(pi))*w(i)*qfunc((dE-i0)./sigmaN_i);
        else 
            Sum(i)=1/(2*sqrt(pi))*w(i)*qfunc((dE-i1)./sigmaN_i);
        end
    end
