function [Result]=jointProbabiliteGammaRf_AE(bit_Alice,bit_Eve,P_dBm,C2n)
    ...
    for i=1:length(t)
        ...
        for l=1:length(t)
            ...
            if bit_Alice==0 && bit_Eve==0
                Sum(i,l)=1/2*a(i)*special(i)^(-b(i))*v(l)*t(l)^(b(i)-1)*qfunc((i0-dE)/sigmaN_i_l);   
            elseif bit_Alice==1 && bit_Eve==0
                Sum(i,l)=1/2*a(i)*special(i)^(-b(i))*v(l)*t(l)^(b(i)-1)*qfunc((i1-dE)/sigmaN_i_l);    
            elseif bit_Alice==0 && bit_Eve==1
                Sum(i,l)=1/2*a(i)*special(i)^(-b(i))*v(l)*t(l)^(b(i)-1)*qfunc((dE-i0)/sigmaN_i_l);      
            else 
                Sum(i,l)=1/2*a(i)*special(i)^(-b(i))*v(l)*t(l)^(b(i)-1)*qfunc((dE-i1)/sigmaN_i_l);
            end
        end
    end
  	Result=sum(sum(Sum));
