...
ScaleCo=1.25;
P_T_dBm1=-3;            %Transmitted power(dBm)-> Value=0||5
P_T_dBm2=-1;
P_T_dBmArr=-10:0.95:10;
...
title('(a) P_{T} = -3 dBm','FontWeight','Normal');
legend('QBER','P_{sift}');
...
ylabel('Probability');
% title(['QBER and P_{sift} versus the threshold scale coefficient (\rho) with P_{LO}=',num2str(P_LO_dBm),' (dBm)']);
title('(b) P_{T} = -1 dBm','FontWeight','Normal');
legend('QBER','P_{sift}');
axis([0,4,1.e-6,1.e-0]);
