...
ScaleCo=1.5;
P_T_dBm1=25;            %Transmitted power(dBm)-> Value=0||5
P_T_dBm2=30;
P_T_dBmArr=0:0.95:35;
...
grid on
xlabel('Threshold scale coefficient, \rho');
ylabel('Probability');
% title(['QBER and P_{sift} versus the threshold scale coefficient (\rho) with P_{LO}=',num2str(P_LO_dBm),' (dBm)']);
title(['(a) P_{T} = ',num2str(P_T_dBm1),' dBm'],'FontWeight','Normal');
legend('QBER','P_{sift}');
...
ylabel('Probability');
% title(['QBER and P_{sift} versus the threshold scale coefficient (\rho) with P_{LO}=',num2str(P_LO_dBm),' (dBm)']);
title(['(b) P_{T} = ',num2str(P_T_dBm2),' dBm'],'FontWeight','Normal');
legend('QBER','P_{sift}');
axis([0,4,1.e-6,1.e-0]);
