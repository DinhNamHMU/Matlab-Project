function [Result]=jointProbabilite_QPSK_S_AE_3D(bit_Alice,bit_Eve,X_L_AE_Array) 
    ...
    %Calculate QBER
    sigmaN2=2.*q.*M.^(2+x).*(P_LO.*R+Id).*deltaf+4.*kB.*T./RL.*deltaf;
    L=L1+X_L_AE_Array;
    La=(H_a-H_G)./cos(zenithAng)+X_L_AE_Array;
    A=pi.*r.^2;
    h=G_Tx.*A./(pi.*(L.*teta).^2).*exp(-alpha.*La./1000).*G_Rx;
    P_R=P_T.*h;
    I0=2.*M.*R.*sqrt(P_R.*P_LO);
    I1=-2.*M.*R.*sqrt(P_R.*P_LO);
