...
P_Direct=zeros(1,length(P_T_dBm));
QBER_SIM=zeros(1,length(P_T_dBm));
P_SIM=zeros(1,length(P_T_dBm));

for i=1:length(P_T_dBm)
    [QBER_Gamma_Weak(i),P_sift_Gamma_Weak(i)]=calculateQBER_QPSK_Gamma(ScaleCo_Weak,P_T_dBm(i),C2n_Weak);
    [QBER_SIM(i),P_SIM(i)]=calculateQBER_BPSK_Gamma(ScaleCo_Weak,P_T_dBm(i),C2n_Weak);
    [QBER_Direct(i),P_Direct(i)]=calculateQBER_QPSK_DD(ScaleCo_Weak,P_T_dBm(i),C2n_Weak);
end
...
semilogy(P_T_dBm,P_SIM,'-o','color',[0, 0, 1],'LineWidth',1.25);
semilogy(P_T_dBm,QBER_Direct,'-x','color',[0, 0.5, 0],'LineWidth',1.25);
semilogy(P_T_dBm,P_Direct,'-d','color',[0, 0.5, 0],'LineWidth',1.25);
