...
delayOutageRate2_Weak=zeros(1,length(Attenuation));
delayOutageRate3_Weak=zeros(1,length(Attenuation));
delayOutageRate4_Weak=zeros(1,length(Attenuation));
delayOutageRate5_Weak=zeros(1,length(Attenuation));
delayOutageRate6_Weak=zeros(1,length(Attenuation));
delayOutageRate7_Weak=zeros(1,length(Attenuation));
delayOutageRate8_Weak=zeros(1,length(Attenuation));
...
delayOutageRate2_Strong=zeros(1,length(Attenuation));
delayOutageRate3_Strong=zeros(1,length(Attenuation));
delayOutageRate4_Strong=zeros(1,length(Attenuation));
delayOutageRate5_Strong=zeros(1,length(Attenuation));
delayOutageRate6_Strong=zeros(1,length(Attenuation));
delayOutageRate7_Strong=zeros(1,length(Attenuation));
delayOutageRate8_Strong=zeros(1,length(Attenuation));

for i=1:length(Attenuation) 
    [QBER_Strong(i),P_sift_Strong(i)]=calculateQBER_QPSK_Gamma(Attenuation(i),ScaleCo_Strong,P_T_dBm,C2n_Strong);
    
    [delayOutageRate2_Strong(i)]=calculateKeyLossRate(MArray(2),B,QBER_Strong(i),P_sift_Strong(i));
    [delayOutageRate3_Strong(i)]=calculateKeyLossRate(MArray(3),B,QBER_Strong(i),P_sift_Strong(i));  
    [delayOutageRate4_Strong(i)]=calculateKeyLossRate(MArray(4),B,QBER_Strong(i),P_sift_Strong(i));
    [delayOutageRate5_Strong(i)]=calculateKeyLossRate(MArray(5),B,QBER_Strong(i),P_sift_Strong(i));
    [delayOutageRate6_Strong(i)]=calculateKeyLossRate(MArray(6),B,QBER_Strong(i),P_sift_Strong(i));
    [delayOutageRate7_Strong(i)]=calculateKeyLossRate(MArray(7),B,QBER_Strong(i),P_sift_Strong(i));
    [delayOutageRate8_Strong(i)]=calculateKeyLossRate(MArray(8),B,QBER_Strong(i),P_sift_Strong(i));
end

%Plot function of delay outage rate
%Weak turbulence
figure(1)
semilogy(Attenuation,delayOutageRate2_Weak,'r--',Attenuation,delayOutageRate3_Weak,'kd-',Attenuation,delayOutageRate4_Weak,'g+-',...
         Attenuation,delayOutageRate5_Weak,'m-*',Attenuation,delayOutageRate6_Weak,'c-o',Attenuation,delayOutageRate7_Weak,'k-s',...
         Attenuation,delayOutageRate8_Weak,'b-x','LineWidth',1.5);
grid on
...
%Strong turbulence
figure(2)
semilogy(Attenuation,delayOutageRate2_Strong,'r--',Attenuation,delayOutageRate3_Strong,'kd-',Attenuation,delayOutageRate4_Strong,'g+-',...
         Attenuation,delayOutageRate5_Strong,'m-*',Attenuation,delayOutageRate6_Strong,'c-o',Attenuation,delayOutageRate7_Strong,'k-s',...
         Attenuation,delayOutageRate8_Strong,'b-x','LineWidth',1.5);
grid on
xlabel('Attenuation coefficient, \sigma (dB/km)');
ylabel('Delay outage rate');
