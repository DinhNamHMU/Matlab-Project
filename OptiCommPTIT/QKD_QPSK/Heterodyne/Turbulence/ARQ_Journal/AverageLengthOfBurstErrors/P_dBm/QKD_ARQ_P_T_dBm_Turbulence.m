...
l_b1_Weak=zeros(1,length(P_T_dBm));
l_b2_Weak=zeros(1,length(P_T_dBm));
l_b3_Weak=zeros(1,length(P_T_dBm));
l_b4_Weak=zeros(1,length(P_T_dBm));
l_b5_Weak=zeros(1,length(P_T_dBm));
l_b6_Weak=zeros(1,length(P_T_dBm));
l_b7_Weak=zeros(1,length(P_T_dBm));
l_b8_Weak=zeros(1,length(P_T_dBm));
...
% 
% l_b1_Strong=zeros(1,length(P_T_dBm));
% l_b2_Strong=zeros(1,length(P_T_dBm));
% l_b3_Strong=zeros(1,length(P_T_dBm));
% l_b4_Strong=zeros(1,length(P_T_dBm));
% l_b5_Strong=zeros(1,length(P_T_dBm));
% l_b6_Strong=zeros(1,length(P_T_dBm));
% l_b7_Strong=zeros(1,length(P_T_dBm));
% l_b8_Strong=zeros(1,length(P_T_dBm));
% 
% for i=1:length(P_T_dBm) 
...
%Weak turbulence
figure(3)
semilogy(P_T_dBm,l_b1_Weak,'b-',P_T_dBm,l_b2_Weak,'r--',P_T_dBm,l_b3_Weak,'kd-',P_T_dBm,l_b4_Weak,'g+-',...
         P_T_dBm,l_b5_Weak,'m-*',P_T_dBm,l_b6_Weak,'c-o',P_T_dBm,l_b7_Weak,'k-s',P_T_dBm,l_b8_Weak,'b-x','LineWidth',1.5);
grid on
xlabel('Peak transmitted power P_{T} (dBm)');
...
% %Strong turbulence
% figure(4)
% semilogy(P_T_dBm,l_b1_Strong,'b-',P_T_dBm,l_b2_Strong,'r--',P_T_dBm,l_b3_Strong,'kd-',P_T_dBm,l_b4_Strong,'g+-',...
%          P_T_dBm,l_b5_Strong,'m-*',P_T_dBm,l_b6_Strong,'c-o',P_T_dBm,l_b7_Strong,'k-s',P_T_dBm,l_b8_Strong,'b-x','LineWidth',1.5);
% grid on
% xlabel('Peak transmitted power P_{T} (dBm)');
% ylabel('Average length of burst errors (packages)');
% legend('Conventional QKD','QKD-KR, M = 1', 'QKD-KR, M = 2', 'QKD-KR, M = 3', 'QKD-KR, M = 4',...
