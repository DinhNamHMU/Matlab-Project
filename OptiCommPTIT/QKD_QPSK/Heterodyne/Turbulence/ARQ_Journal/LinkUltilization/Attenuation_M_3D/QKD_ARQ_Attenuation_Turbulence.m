...
for i=1:length(M_transmission) 
    for j=1:length(M_transmission) 
        ...
        %Link utilization
        linkUtilization_Strong(i,j)=lamda*(1-keyLossRate_Strong(i,j))/(Rb/l_k);
    end
end
...
% %Strong turbulence
% figure(1)
% surfc(X_retransmission,Y_AttenuationArray,linkUtilization_Strong);
...
figure(2)
contourf(X_retransmission,Y_AttenuationArray,linkUtilization_Strong);
grid on
ax=gca;
ax.GridColor='White';
ax.GridLineStyle=':';
