...
linkUtilization_Strong=zeros(length(LamdaArray),length(LamdaArray));
keyLossRate_Strong=zeros(length(LamdaArray),length(LamdaArray));
...
for i=1:length(LamdaArray) 
    for j=1:length(LamdaArray) 
        ...
        %Key loss rate
        keyLossRate_Strong(i,j)=calculateKeyLossRate(MArray(2),B,QBER_Strong(i,j),P_sift_Strong(i,j),X_LamdaArray(i,j));
        
        %Link utilization
        linkUtilization_Strong(i,j)=X_LamdaArray(i,j)*(1-keyLossRate_Strong(i,j))/(Rb/l_k);
    end
end
...
% %Strong turbulence
% figure(1)
% surfc(X_LamdaArray,Y_AttenuationArray,linkUtilization_Strong);
% xlabel('Buffer size, B (bit strings)');
% ylabel('Attenuation coefficient, \sigma (dB/km)');
% zlabel('Link utilization, U');
% colorbar;
% colormap('jet');

figure(2)
contourf(X_LamdaArray,Y_AttenuationArray,linkUtilization_Strong);
grid on
ax=gca;
ax.GridColor='White';
ax.GridLineStyle=':';
...
ylabel('Attenuation coefficient, \sigma (dB/km)');
zlabel('Link utilization, U');
title('Link utilization, U','FontWeight','Normal');
colorbar
colormap('jet');