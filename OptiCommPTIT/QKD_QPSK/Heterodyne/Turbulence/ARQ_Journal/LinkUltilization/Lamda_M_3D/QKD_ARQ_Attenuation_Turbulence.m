...
M_retransmission=linspace(0,7,scale);
...
linkUtilization_Strong=zeros(length(M_transmission),length(M_transmission));
...
for i=1:length(M_transmission) 
    for j=1:length(M_transmission) 
        ...
        %Key loss rate
        keyLossRate_Strong(i,j)=calculateKeyLossRate(X_transmission(i,j),B,QBER_Strong(i,j),P_sift_Strong(i,j),Y_LamdaArray(i,j));
        
        %Link utilization
        linkUtilization_Strong(i,j)=Y_LamdaArray(i,j)*(1-keyLossRate_Strong(i,j))/(Rb/l_k);
    end
end
...
% %Strong turbulence
% figure(1)
% surfc(X_retransmission,Y_LamdaArray,linkUtilization_Strong);
...
figure(2)
contourf(X_retransmission,Y_LamdaArray,linkUtilization_Strong);
grid on
ax=gca;
ax.GridColor='White';
ax.GridLineStyle=':';
